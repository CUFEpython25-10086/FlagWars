...
class GameWebSocketHandler(websocket.WebSocketHandler):
    ...
    def initialize(self, game_manager):
        ...
        self.is_ai_controlled = False  # 是否为AI控制的玩家
    ...
    def on_message(self, message):
        ...
        try:
            ...
            if message_type == 'join_game':
                ...
                self._handle_add_ai_player(data)
            elif message_type == 'remove_ai_player':
                self._handle_remove_ai_player(data)
    ...
    def _handle_add_ai_player(self, data):
        """处理添加AI玩家请求"""
        if not self.game_id:
            self.send_error("请先创建或加入房间")
            return

        # 检查是否有添加AI玩家的权限（房主）
        game_state = self.game_manager.games.get(self.game_id)
        if not game_state:
        ...
        # 检查是否是第一个加入的玩家（房主）
        if self.player_id != 1:
            self.send_error("只有房主可以添加AI玩家")
            return

        # 检查游戏是否已经开始
        if game_state.game_started:
            self.send_error("游戏已经开始，无法添加AI玩家")
            return

        # 检查房间是否已满
        if len(game_state.players) >= 8:
            self.send_error("房间已满，无法添加AI玩家")
            return

        # 获取AI配置
        ai_name = data.get('ai_name', 'AI玩家')
        aggression = data.get('aggression', 0.6)
        ai_type = data.get('ai_type', 'simple')  # AI类型：simple, aggressive, defensive

        # 添加AI玩家
        success, ai_player_id, error_msg = self.game_manager.add_ai_player(
            self.game_id, ai_name, aggression, ai_type
        )
        ...
        if success:
            response = {
                ...
                'ai_name': ai_name,
                'message': f'AI玩家 {ai_name} 已加入游戏',
                'game_state': self.game_manager.get_game_state(self.game_id)
            }
            self.write_message(json.dumps(response, default=str))

            # 广播给其他玩家
            self.game_manager.broadcast_ai_player_added(self.game_id, ai_name, ai_player_id)
        else:
            response = {
                'type': 'add_ai_failed',
                'message': error_msg
    ...
    def _handle_remove_ai_player(self, data):
        """处理移除AI玩家请求"""
        if not self.game_id:
            self.send_error("请先创建或加入房间")
            return
        ...
        # 检查是否是第一个加入的玩家（房主）
        if self.player_id != 1:
            self.send_error("只有房主可以移除AI玩家")
            return

        # 检查游戏是否已经开始
        if game_state.game_started:
            self.send_error("游戏已经开始，无法移除AI玩家")
            return

        ai_player_id = data.get('ai_player_id')
        if not ai_player_id:
            self.send_error("请指定要移除的AI玩家ID")
            return

        # 移除AI玩家
        success, error_msg = self.game_manager.remove_ai_player(self.game_id, ai_player_id)
        ...
        if success:
            ...
            # 广播给其他玩家
            self.game_manager.broadcast_ai_player_removed(self.game_id, ai_player_id)
        else:
            response = {
                'type': 'remove_ai_failed',
                'message': error_msg
            }
            self.write_message(json.dumps(response))
    ...
    def _handle_leave_room(self):
        ...
        if not self.game_id or not self.player_id:
            self.send_error("您不在任何房间中")
            return

        # 保存房间ID和玩家ID，因为leave_game会清除它们
        game_id = self.game_id
        player_id = self.player_id

        # 通知游戏管理器玩家离开
        self.game_manager.leave_game(game_id, player_id)
...
class GameManager:
    ...
    def __init__(self):
        ...
        # AI相关属性
        self.ai_controllers: Dict[str, Dict[int, SimpleAI]] = {}  # AI控制器
        self.ai_enabled = AI_AVAILABLE  # AI功能是否启用
        self.ai_player_names = [  # AI玩家名称列表
        ...
        if self.ai_enabled:
            logging.info("✅ AI功能已启用")
        else:
            logging.warning("⚠️  AI功能不可用，请确保simple_ai.py存在")
    ...
    def create_room(self) -> str:
        ...
        # 创建新游戏实例
        game_state = GameState(enable_ai=self.ai_enabled)
        self.games[room_id] = game_state
        self.players[room_id] = {}
        self.player_ready_states[room_id] = {}
        self.ai_controllers[room_id] = {}  # 初始化AI控制器字典
    ...
    def join_room(self, room_id: str, player_name: str, user_id: int = None) -> tuple:
        ...
        player = Player(player_id, player_name, player_color, is_ai=False)
    ...
    def add_ai_player(self, room_id: str, ai_name: str = None, aggression: float = 0.6,
                     ai_type: str = 'simple') -> tuple:
        """
        添加AI玩家到房间
        ...
        Args:
            ...
            ai_type: AI类型 ('simple', 'aggressive', 'defensive')

        Returns:
            (success, ai_player_id, error_message)
        """
        if not self.ai_enabled:
            return False, None, "AI功能未启用"

        # 检查房间是否存在
        if room_id not in self.games:
            return False, None, "房间不存在"

        # 检查房间是否已开始
        if self.games[room_id].game_started:
            return False, None, "游戏已开始，无法添加AI玩家"
        ...
        # 分配AI玩家ID
        ai_player_id = self.next_player_id
        self.next_player_id += 1

        # 生成AI玩家名称
        if not ai_name:
            # 统计当前AI玩家数量，生成唯一名称
            ai_count = len(self.ai_controllers.get(room_id, {}))
            if ai_count < len(self.ai_player_names):
                ai_name = self.ai_player_names[ai_count]
            else:
                ai_name = f"AI-{ai_count + 1}"
        ...
        # 创建AI玩家
        ai_player = Player(ai_player_id, ai_name, player_color, is_ai=True)
        ...
        # 添加AI玩家到游戏状态
        game_state.add_player(ai_player, base_x, base_y)
        self.player_ready_states[room_id][ai_player_id] = False  # AI玩家初始未准备

        # 初始化AI控制器
        if room_id not in self.ai_controllers:
            self.ai_controllers[room_id] = {}

        # 创建AI控制器
        ai_controller = SimpleAI(player_id=ai_player_id, aggression=aggression)
        ai_controller.connect_to_game(game_state)
        self.ai_controllers[room_id][ai_player_id] = ai_controller

        # 添加AI玩家到玩家连接字典（设置为None，因为AI没有WebSocket连接）
        if room_id not in self.players:
            self.players[room_id] = {}
        self.players[room_id][ai_player_id] = None

        logging.info(f"AI玩家 {ai_name} (ID: {ai_player_id}, 攻击性: {aggression}) 已添加到房间 {room_id}")

        return True, ai_player_id, None

    def remove_ai_player(self, room_id: str, ai_player_id: int) -> tuple:
        """
        从房间移除AI玩家
        ...
        # 检查游戏是否已经开始
        if self.games[room_id].game_started:
            return False, "游戏已经开始，无法移除AI玩家"

        # 检查AI玩家是否存在
        if room_id not in self.ai_controllers or ai_player_id not in self.ai_controllers[room_id]:
            return False, "AI玩家不存在"

        # 从游戏状态中移除玩家
        self.games[room_id].remove_player(ai_player_id)

        # 移除准备状态
        if room_id in self.player_ready_states and ai_player_id in self.player_ready_states[room_id]:
            del self.player_ready_states[room_id][ai_player_id]

        # 移除AI控制器
        del self.ai_controllers[room_id][ai_player_id]

        # 移除玩家连接
        if room_id in self.players and ai_player_id in self.players[room_id]:
            del self.players[room_id][ai_player_id]

        logging.info(f"AI玩家 {ai_player_id} 已从房间 {room_id} 移除")
    ...
    def broadcast_ai_player_added(self, game_id: str, ai_name: str, ai_player_id: int):
        """广播AI玩家加入消息给所有玩家"""
        if game_id not in self.players:
            return

        message = {
            'type': 'ai_player_added_notification',
            'ai_player_id': ai_player_id,
            'ai_name': ai_name,
            'message': f'AI玩家 {ai_name} 已加入游戏',
            'game_state': self.get_game_state(game_id)
        ...
        for player_id, handler in self.players[game_id].items():
            # 只向有WebSocket连接的玩家广播（即排除AI玩家）
            if handler:
                handler.write_message(json.dumps(message, default=str))

    def broadcast_ai_player_removed(self, game_id: str, ai_player_id: int):
        """广播AI玩家移除消息给所有玩家"""
        if game_id not in self.players:
            return

        message = {
            'type': 'ai_player_removed_notification',
            'ai_player_id': ai_player_id,
            'message': 'AI玩家已移除',
            'game_state': self.get_game_state(game_id)
        }
        ...
        for pid, player in game_state.players.items():
            if player.is_ai:
                ai_players.append({
                    'player_id': pid,
                    'name': player.name,
                    'aggression': player.ai_aggression if hasattr(player, 'ai_aggression') else 0.6
                })
        state_dict['ai_players'] = ai_players
    ...
    def _update_all_games(self):
        ...
        for game_id, game_state in self.games.items():
            ...
            # AI决策：让AI玩家做出决策
            if game_state.game_started and game_id in self.ai_controllers:
                for ai_player_id, ai_controller in self.ai_controllers[game_id].items():
                    if (ai_player_id in game_state.players and
                        game_state.players[ai_player_id].is_alive and
                        not game_state.players[ai_player_id].is_spectator):
                        try:
                            # AI做出决策
                            ai_controller.decide_and_act()
                        except Exception as e:
                            logging.error(f"AI玩家 {ai_player_id} 决策出错: {e}")
    ...
    def close_room(self, room_id: str):
        ...
        if room_id in self.ai_controllers:
            del self.ai_controllers[room_id]
    ...
    def reset_game(self, game_id: str) -> bool:
        ...
        # 创建新的游戏状态
        new_game_state = GameState(enable_ai=self.ai_enabled)
        ...
        # 重新初始化AI控制器
        if game_id in self.ai_controllers:
            for ai_player_id in self.ai_controllers[game_id]:
                if ai_player_id in new_game_state.players:
                    ai_controller = self.ai_controllers[game_id][ai_player_id]
                    ai_controller.connect_to_game(new_game_state)
...
class AITestHandler(web.RequestHandler):
    """AI测试页面处理器"""

    def get(self):
        """提供AI对战测试页面"""
        import os
        template_path = os.path.join(os.path.dirname(__file__), 'templates', 'ai_test.html')
        with open(template_path, 'r', encoding='utf-8') as f:
            self.write(f.read())
...
def make_app():
    ...
    routes = [
        ...
        (r"/ai-test", AITestHandler),  # 添加AI测试页面
...
def main():
    ...
    logging.info(f"AI对战测试页面: http://localhost:{args.port}/ai-test")
